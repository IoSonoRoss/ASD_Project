# File: path_utils.py (Versione RICORSIVA e Definitiva)
...
def generate_path_coordinates(origin, destination, diagonal_first):
    ...
    num_diag, num_rect = min(abs(delta_r), abs(delta_c)), abs(abs(delta_r) - abs(delta_c))
...
# --- PROCEDURA CAMMINOMIN PURA (SENZA ETICHETTE) ---
memoization_cache = {}

def procedura_cammino_min(origin, destination, grid, ostacoli_proibiti=frozenset()):
    """
    [Versione Semplice e Pura] Implementazione RICORSIVA che lavora solo con coordinate.
    """
    ...
    for F, tipo_F in frontiera_con_tipo:
        ...
        if lOF + calcola_distanza_libera(F, destination) >= lunghezza_min:
            continue

        chiusura_attuale = set(contesto) | set(complemento) | {origin}
        nuovi_ostacoli_proibiti = ostacoli_proibiti.union(chiusura_attuale)
        
        lFD, seqFD = procedura_cammino_min(F, destination, grid, frozenset(nuovi_ostacoli_proibiti))
